...
def preprocess_image(image_array):
    """Preprocess image for OCR - optimized for black background with white text"""
    img_cv = cv2.cvtColor(np.array(image_array), cv2.COLOR_RGB2BGR)
    gray = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)
    
    # Invert if dark background (most pixels are dark)
...
def extract_options_data(image):
    ...
    try:
        ...
        # Better OCR config for numbers
        custom_config = r'--oem 3 --psm 6 --dpi 300 -c tessedit_char_whitelist=0123456789,'
        data = pytesseract.image_to_data(processed, 
                                       output_type=pytesseract.Output.DICT,
                                       config=custom_config)
        
        # Group by rows - be more strict about what's on the same row
        rows = {}
        tolerance = 15  # Tighter tolerance to avoid merging rows
        
        for i, text in enumerate(data['text']):
            # Accept lower confidence but require actual content
            if text.strip() and data['conf'][i] >= 0:
                ...
                rows[row_key].append({'text': text, 'x': x, 'conf': data['conf'][i]})
        ...
        for row_y in sorted(rows.keys()):
            ...
            # Only take rows with exactly 2 numbers (volume and OI)
            if len(numbers) == 2:
                volume = numbers[0]
                oi = numbers[1]
                options_data.append({
